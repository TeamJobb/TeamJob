...
import { FaEnvelope, FaCommentDots, FaUser, FaHome } from 'react-icons/fa'; // Importer FaHome
  ...
  return (
    <>
      <nav className="navbar navbar-expand-lg navbar-light bg-white py-2 sticky-top">
        <Link className="navbar-brand text-info font-weight-bold" to="/"> {/* Ajouter font-weight-bold */}
          <FaHome /> Home Page
        </Link>
            ...
            {user && (
              <li className="nav-item active">
                <Link className="nav-link" to={`/profile/${user.id}`}>
                  <FaUser /> Your Profile <span className="sr-only">(current)</span>
                </Link>
              </li>
            )}
